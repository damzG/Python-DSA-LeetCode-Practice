...
class LinkedList:
    ...
    def remove_duplicates(self):
        ...
        #Start the current at head node to check each node value for duplicates
        current = self.head
        
        #visits every node in the linked list
        while current:
            runner = current
            
            #Loop while runner.next exists to check if the next node's value is equals to current.value
            while runner.next:
                if runner.next.value == current.value:
                    runner.next = runner.next.next #skip the duplicate by linking to the one after it 
                    self.length -= 1 # decrease the list 
                else:
                    runner = runner.next #move to the next node to keep checking
            #Move current to the next node to check for duplicates
            current = current.next
